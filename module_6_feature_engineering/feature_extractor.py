"""
=============================================================================
MODULE 6 – FEATURE ENGINEERING  |  feature_extractor.py
=============================================================================
...
Feature coverage (93 total):
  EDA:     22 features  (10 SCL/SCR derived + 8 time-domain + 3 freq + 1 non-linear)
  BVP:     16 features  (7 beat morphology + 4 time-domain + 4 freq + 1 SQI)
  IBI/HRV: 17 features  (5 time-domain + 7 freq-domain + 5 non-linear)
  ST:      12 features  (5 derived + 4 time-domain + 2 freq + 1 autocorrelation)
  ACC:     26 features  (8 derived + 6 per-axis std/zcr + 5 freq bands + 3 cross-corr
                         + 3 time-domain + 1 non-linear)
...
def _butterworth_lowpass(sig: np.ndarray, fs: float, cutoff: float,
                         order: int = 4) -> np.ndarray:
    """Zero-phase Butterworth low-pass filter (local copy for EDA decomposition)."""
    nyq = fs / 2.0
...
def _decompose_eda(eda: np.ndarray, fs: float):
    ...
    tonic = _butterworth_lowpass(eda, fs, cutoff=0.05, order=2)
    phasic = eda - tonic
...
def extract_eda_features(sig: np.ndarray, fs: float,
    ...
    f["eda_scl_delta"] = _safe(np.mean(tonic) - base)
...
def extract_bvp_features(sig: np.ndarray, fs: float) -> dict:
    ...
    # Harmonic ratio: power at fundamental freq vs total — measures waveform regularity
    f0 = _dominant_freq(sig, fs, 0.5, 3.5)
    if np.isfinite(f0) and f0 > 0:
        fund_pow = _band_power(sig, fs, max(0.5, f0 - 0.3), f0 + 0.3)
        total_pow = _band_power(sig, fs, 0.5, fs / 2)
        f["bvp_harm_ratio"] = _safe(fund_pow / (total_pow + 1e-12))
    else:
        f["bvp_harm_ratio"] = np.nan
