"""
=============================================================================
MODULE 7 -- MODEL TRAINING  |  models/bilstm_forecaster.py
=============================================================================
BiLSTM Time Series Forecaster -- predicts the emotional state N minutes
into the future using M minutes of physiological signal history.

Architecture mirrors the classification BiLSTM but with a critical
difference: the training label comes from a FUTURE window (horizon steps
ahead) rather than the current/last window.

Default: 3-minute lookback (6 windows) -> predict 1 minute ahead (2 windows)
         with 30-second effective stride (1 window step).
=============================================================================
"""
...
from config import (
    BILSTM_FORECAST, MIN_FORECAST_SEQUENCES, RANDOM_SEED,
...
class BiLSTMForecastModel(nn.Module):
    """
    Bidirectional LSTM for time series emotional state forecasting.

    Input:  (batch, lookback, n_features) -- history windows
    Output: (batch, n_classes) -- predicted future emotional state
    """
...
class BiLSTMForecasterTrainer(BaseModelTrainer):
    ...
    def can_run(self, data) -> tuple:
        ...
        if n_train < MIN_FORECAST_SEQUENCES:
            return False, (f"Only {n_train} forecast sequences "
                           f"(need >= {MIN_FORECAST_SEQUENCES})")
        if len(np.unique(data.y_train)) < 2:
            return False, "Only 1 class in forecast training data"
        return True, ""
    ...
    def train(self, data, output_dir: Path, **kwargs) -> TrainResult:
        ...
        cfg = BILSTM_FORECAST
        self._log(f"Training BiLSTM Forecaster on {self.device} ...")
        self._log(f"  Lookback={data.lookback} windows (3 min), "
                   f"Horizon={data.horizon} windows (1 min ahead)")
        t0 = time.time()
        ...
        self.model = BiLSTMForecastModel(
        ...
        # Add forecast-specific metadata to metrics
        metrics["forecast_lookback_windows"] = data.lookback
        metrics["forecast_horizon_windows"] = data.horizon
        metrics["forecast_lookback_seconds"] = data.lookback * 30  # 30s stride
        metrics["forecast_horizon_seconds"] = data.horizon * 30
        metrics["forecast_description"] = (
            f"{data.lookback * 30}s history -> predict {data.horizon * 30}s ahead"
        )

        ev.plot_confusion_matrix(
            data.y_test, y_pred,
            output_dir / "confusion_matrix.png",
            title="BiLSTM Forecaster - Confusion Matrix\n"
                  f"(3-min history -> 1-min ahead)")
        ev.plot_training_curves(
            history, output_dir / "training_curves.png",
            title="BiLSTM Forecaster - Training Curves")
        if y_prob is not None:
            ev.plot_roc_curves(
                data.y_test, y_prob,
                output_dir / "roc_curves.png",
                title="BiLSTM Forecaster - ROC Curves")

        # Save forecast config for M8 to load
        forecast_meta = {
            "model_type": "forecaster",
            "lookback_windows": data.lookback,
            "horizon_windows": data.horizon,
            "lookback_seconds": data.lookback * 30,
            "horizon_seconds": data.horizon * 30,
            "n_features": n_features,
            "n_classes": data.n_classes,
            "class_names": data.class_names,
        }
        with open(output_dir / "forecast_config.json", "w") as f:
            json.dump(forecast_meta, f, indent=2)

        self._log(f"  Test F1={metrics.get('f1_weighted', 0):.4f}, "
                   f"Acc={metrics.get('accuracy', 0):.4f} ({elapsed:.1f}s)")

        return TrainResult(
            model_name=self.name, model_type=self.model_type,
            framework=self.framework, metrics=metrics,
            model_path=model_path, training_time_s=elapsed,
            hyperparams={**cfg, "lookback": data.lookback,
