"""
=============================================================================
MODULE 7 -- MODEL TRAINING  |  models/decision_tree.py
=============================================================================
Decision Tree classifier -- interpretable baseline with minimal tuning.
=============================================================================
"""
...
from config import DT_PARAM_GRID, CV_FOLDS, CV_SCORING, RANDOM_SEED
...
class DecisionTreeTrainer(BaseModelTrainer):
    ...
    def can_run(self, data) -> tuple:
        ...
        if n_classes < 2:
            return False, f"Only {n_classes} class(es) in training set"
        return True, ""
    ...
    def train(self, data, output_dir: Path, **kwargs) -> TrainResult:
        ...
        self._log("Training Decision Tree with GridSearchCV ...")
        t0 = time.time()
        ...
        base = DecisionTreeClassifier(random_state=self.seed)
        grid = GridSearchCV(
            base, DT_PARAM_GRID, cv=cv, scoring=CV_SCORING,
